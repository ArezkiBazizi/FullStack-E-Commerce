$(document).ready(function(){
	...
	*/
	...
	//afficher le produit seul
	$("body").delegate("#viewproduct","click",function(){
		var pid = $(this).attr('pid');
			$.ajax({
			url		:	"action.php",
			method	:	"POST",
			data	:	{view_product:1,product_id:pid},
			success	:	function(data){
				$("#apercu").html(data);
				$("#apercu").css('display','flex');
			}
			})
	})
	...
	*/
	...
	//Add Product into Favoris
	$("body").delegate("#favproduct","click",function(event){
		var pid = $(this).attr("pid");
		event.preventDefault();
		$(".overlay").show();
		$.ajax({
	...
	//Add Product into Favoris end here
	...
	count_fav_item();
	function count_fav_item(){
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {count_fav_item:1},
			success : function(data){
				$("#favcount").html(data);
			}
		})
	...
	getFavorisItem();
	function getFavorisItem(){
		$.ajax({
			url : "action.php",
			method : "POST",
			data : {getFavorisItem:1},
			success : function(data){
				$("#favList").html(data);
			}
		})
	...
	*/
	$("body").delegate(".qty","keyup",function(event){
		...
		$('.net_total').html("Total : " +net_total+" DZ");
	...
	*/
	...
	$("body").delegate(".closeFav","click",function(event){
		var remove = $(this).parent();
		var remove_id = remove.find(".closeFav").attr("remove_id");
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{removeItemFromFavoris:1,rid:remove_id},
			success	:	function(data){
				$("#cart_msg").html(data);
				getFavorisItem();
				count_fav_item();
	...
	/*
		vider le panier
	*/
	$("body").delegate("#vider","click",function(event){
		$.ajax({
			url	:	"action.php",
			method	:	"POST",
			data	:	{removeallCart:1},
			success	:	function(data){
				$("#cart_msg").html(data);
				checkOutDetails();
				count_item();
